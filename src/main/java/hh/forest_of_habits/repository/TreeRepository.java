...
public interface TreeRepository extends CrudRepository<Tree, Long> {
    @Query("SELECT new hh.forest_of_habits.dto.response.TreeResponse(t.id, " +
            "t.name, t.description, t.type, t.period, t.createdAt, t.limit, t.forest.id, coalesce(SUM(il.value), 0)) " +
            "FROM Tree t LEFT JOIN t.increments il " +
            "WHERE t.forest.id = :forestId AND " +
            "(t.type <> 'PERIODIC_TREE' OR (t.type = 'PERIODIC_TREE' AND " +
            "FUNCTION('AGE', coalesce(il.date, current_date), current_date) <= CASE t.period " +
