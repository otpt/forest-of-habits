...
public interface TreeRepository extends CrudRepository<Tree, Long> {
    ...
    @Query("SELECT new hh.forest_of_habits.dto.response.TreeResponse(t.id, t.name, t.description, t.type, t.period, t.createdAt, t.limit, t.forest.id, coalesce(SUM(il.value), 0)) " +
            ...
            "WHERE t.forest.id = :forestId AND (t.type <> 'PERIODIC_TREE' OR (t.type = 'PERIODIC_TREE' AND FUNCTION('AGE', il.date, current_date) <= CASE t.period " +
            "WHEN hh.forest_of_habits.enums.TreePeriod.DAY THEN FUNCTION('MAKE_INTERVAL', '0', '0', '1') " +
            "WHEN hh.forest_of_habits.enums.TreePeriod.WEEK THEN FUNCTION('MAKE_INTERVAL', '0', '0', '7') " +
